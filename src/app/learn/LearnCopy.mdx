...
- [_`src/app/`_ folder structure](#srcapp-folder-structure)
  ...
  - [_`main-nav`_ routing module](#main-nav-routing-module)
...
[OpenTask](https://opentask.app) is a free and [open-source](https://github.com/flsilva/opentask) task management app created as part of a Next.js App Router case study (a link will be added soon), and it currently implements Nexar beta as the codebase in the feb-2024 branch (a link will be added soon).
...
Next.js uses a [file-based routing system](https://nextjs.org/docs/app/building-your-application/routing/defining-routes), where subfolders of the _`src/app/`_ folder define route segments. Each route segment is mapped to a corresponding segment in a URL path. Those route segments define the entry points of our application, the URLs.
...
You might ask why have a separate _`src/features`_ folder. Why not put everything inside _`src/app`_?

Since we define our public application entry points in _`src/app`_, we might have many related but separated entry points for similar features, campaigns, etc. After some time, they might get cluttered. Those are our **routing** modules.

But we should structure the code powering those routes to reflect the meaningful business concepts behind the application. So, we use a separate _`src/features`_ folder to build our **business feature** modules.
...
#### _`main-nav`_ routing module

The following is the folder structure of the _`src/app/app/main-nav/`_ routing module, defining a single main navigation route segment:

```shell
src/app/app/@dialog/(.)main-menu/page.tsx
src/app/app/main-nav/page.tsx
```

This page is accessed only from mobile devices, opening the main navigation fullscreen in a dialog for a better user experience. On large screens, the main navigation is always visible as a sidebar.
...
We have a layered architecture inside each feature module for the three main layers of Nexar: [Domain](/learn/#domain-layer), [Data Access](/learn/#data-access-layer), and [Presentation](/learn/#presentation-layer).

Each file in a feature module implements the logic of a single layer. We cannot mix the code of different layers in the same file.
...
<InfoBox>
  The Data Access layer should never depend on the Presentation layer.
</InfoBox>

#### Presentation layer

I've worked on many React applications where I chose to have separate Application and Presentation layers. Sometimes, it was overkill; other times, it was necessary for the project's success.

Such separation allows the Presentation layer to provide highly reusable, dumb UI components that only implement UI-related logic and behavior.
...
When I started OpenTask, I experimented with different approaches, including separating Application and Presentation logic. However, since we can eliminate all client-side data fetching and application-level state management logic with the new App Router, a separate Application layer proved to bring little to no value.

Instead, in Nexar and so OpenTask, the Presentation layer has two kinds of React components:
...
The Presentation layer depends on the Data Access layer to fetch data and call Server Actions.

The Presentation layer depends on the Domain layer to enforce some business rules, like validating forms for faster UX (we should always validate forms and data on the server regardless of client validation).

We can have functions in the Presentation layer that are not React components.

OpenTask has the following function:

```shell
src/features/app/tasks/presentation/formatTaskDueDate.ts
```

It receives two date objects and returns a string. It's not a React component. Nonetheless, its logic belongs to the Presentation layer.

<InfoBox>
  It's also worth noting that files in the routing modules, i.e., in the
  Next.js' _`src/app`_ folder, also belong to the Presentation layer.
  _`page.tsx`_, _`layout.tsx`_, _`loading.tsx`_, _`error.tsx`_, etc, are all
  React components that belong to the Presentation layer. We must define them
  separately from the rest of the Presentation components that live in the
  _`src/features`_ folder, but that's just a technical detail we must live with.
</InfoBox>
...
Next.js is a comprehensive framework with a complete routing solution. That's why most apps won't need a navigation layer at all.

In OpenTask, we only have a small routing utility I built at _`src/features/shared/navigation`_. You can read about it here (a link will be added soon).

However, a navigation layer might be necessary for complex cases.

I once led the development of a [CRA](https://create-react-app.dev/)-based cross-platform [React Native Web](https://necolas.github.io/react-native-web/) app. I put together a modular and layered architecture with a navigation layer that used a navigation solution for iOS and Android and another for the web. That allowed the team to isolate platform-specific code in the navigation layer and reuse the rest of the codebase almost entirely among all three platforms.
...
Building a cross-platform Next.js App Router app using React Server Components and a hybrid component tree would be fantastic. In such cases, a navigation layer might be necessary to support different navigation strategies.

If we have a concrete case, we might see how a navigation layer fits into Nexar.
...
The following is the folder structure of the _`src/features/shared/navigation/`_ layer:

```shell
navigation/RouterActions.tsx
navigation/useIsPathActive.ts
navigation/useRouterAction.ts
```

The following is the folder structure of the _`src/features/shared/presentation/`_ layer:
...
To keep it brief, I've omitted presentation files. You can see them here (a link will be added soon).
...
```shell
shared/presentation/Footer.tsx
shared/presentation/Header.tsx
shared/presentation/HeroCopy.tsx
shared/presentation/HeroHeading.tsx
shared/presentation/HeroShell.tsx
shared/presentation/MainMenu.tsx
shared/presentation/MainMenuMobile.tsx
```
...
We could've opted not to have a _`presentation`_ layer folder and have the code in the module's root at _`shared/`_ for simplicity since that's the only layer of code we have here.
...
```shell
app/shared/presentation/Header.tsx
app/shared/presentation/MainNav.tsx
app/shared/presentation/MainNavLink.tsx
```
...
We could have a folder for each component above, like _`presentation/header`_ and _`presentation/main-nav`_. However, since there are fewer components, it's okay to have all of them share the same _`presentation`_ folder. Again, these are personal preferences, not recommendations.
...
```shell
app/projects/data-access/ProjectDataAccess.ts

app/projects/domain/ProjectDomain.ts

app/projects/presentation/ProjectForm.tsx
app/projects/presentation/ProjectList.tsx
app/projects/presentation/ProjectListItem.tsx
```

Not all presentation files are listed above; you can check them here (a link will be added soon).

In OpenTask, the _`ProjectDomain.ts`_ implements business logic for the _project_ business concept in a task management application.

Naming the file _`ProjectDomain.ts`_ is not a recommendation; you can use a different name, and please share your preferences. Besides, a single file works well if we don't have a lot of business logic. Having more than one file would require changing this naming.

OpenTask is very simple, and the implementation of _`ProjectDomain.ts`_ reflects that simplicity: data validation to create and update projects using [zod](https://zod.dev/).

We can add pure functions in that file to implement additional business logic if needed. For example, if we had support for team members, we could have a _`canUserDeleteProject(user, project)`_ pure function to validate if users can delete projects they're members of but not owners.

The _`ProjectDataAccess.ts`_ implements the data access logic for projects; it's a classic CRUD implementation.

It also implements [Data Transfer Object (DTO)](https://martinfowler.com/eaaCatalog/dataTransferObject.html) TypeScript types [inferred from Zod schemas](https://github.com/colinhacks/zod#type-inference).

Naming the file _`ProjectDataAccess.ts`_ is not a recommendation either; you can use a different name. Besides, a single file works well if we don't have a lot of data access logic. Having more than one file would require changing this naming.
...
```shell
app/tasks/data-access/TaskDataAccess.ts

app/tasks/domain/TaskDomain.ts

app/tasks/presentation/TaskForm.tsx
app/tasks/presentation/TaskList.tsx
app/tasks/presentation/TaskListItem.tsx
```

Not all presentation files are listed above; you can check them here (a link will be added soon).

This module is implemented in the same way as the _`projects`_ business feature module we just saw above, i.e., _`TaskDomain.ts`_ implements data validation to create and update tasks using [zod](https://zod.dev/), and _`TaskDataAccess.ts`_ is a classic CRUD implementation and also implements DTOs.
...
```shell
app/today/presentation/TodayPageHeader.tsx
```
...
```shell
app/settings/account/presentation/AccountSettings.tsx
app/settings/presentation/SettingsMenu.tsx
```
...
app/users/presentation/UpdateUserTimeZone.tsx
```
